<template>
    <table class="text-sm w-full mt-5 mb-4">
        ...
        <tbody>
            <tr v-on:click="update(user)" v-for="(user, i) in dataForm.data" :key="i" class="border-gray-600 hover:bg-gray-100">
    ...
    <div class="flex justify-end">
        <button v-on:click="add" class="hover:text-white hover:border-black hover:bg-gray-900 border-2 border-gray-900 px-3 py-1 rounded-md shadow">Add User</button>
    </div>
                ...
                <form method="post" @submit.prevent @submit="saveRecord">
                                ...
                                <div class="mt-5 w-full">
                                    <div class="block">
                                        <div class="block text-left lg:flex lg:space-x-3 w-full">
                                            ...
                                            <input type="text" required name="name" v-model="name" maxlength="100" class="text-sm rounded-md h-7 w-full">
                                        </div>
                                    </div>
                                    <div class="block mt-3">
                                        <div class="block text-left lg:flex lg:space-x-3 w-full">
                                            <label class="w-36" for="">Email Address</label>
                                            <input type="email" required name="email" v-model="email" :disabled="dataForm.update" maxlength="100" class="text-sm rounded-md h-7 w-full">
                                        </div>
                                    </div>
                                    <div class="block mt-3">
                                        <div class="block text-left lg:flex lg:space-x-3 w-full">
                                            <label class="w-36" for="">Role</label>
                                            <select name="role" id="" :disabled="dataForm.update && role == 1" v-model="role" required class="text-sm px-2 py-1 rounded-md h-7 w-full">
                    ...
                    <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse justify-between mt-2">
                        <div>
                            ...
                            <button v-if="!dataForm.update" class="mt-1 w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-blue-600 text-base font-medium text-white hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 sm:ml-3 sm:w-auto sm:text-sm">
                            Save
                            </button>
...
export default {
    ...
    beforeMount() {
        ...
        this.header.path = 'User Management > Users';
    ...
    methods: {
        ...
        update(user)
        {
            if(user.id == 1)
            {
                ...
                this.systemAlert.message = 'You cannot edit Administrator account';
            }
            else
            {
                this.id = user.id;
                ...
                this.dataForm.title = "Update User"
                this.dataForm.show = true;
