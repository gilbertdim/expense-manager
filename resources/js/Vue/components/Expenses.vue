<template>
    <table class="text-sm w-full mt-5 mb-4">
        ...
        <tbody>
            <tr v-on:click="update(expense)" v-for="(expense, i) in dataForm.data" :key="i" class="border-gray-600 hover:bg-gray-100">
    ...
    <div class="flex justify-end">
        <button v-on:click="add" class="hover:text-white hover:border-black hover:bg-gray-900 border-2 border-gray-900 px-3 py-1 rounded-md shadow">Add Expense</button>
    </div>
                                ...
                                <div class="mt-5 w-full">
                                    <div class="block">
                                        <div class="block text-left lg:flex lg:space-x-3 w-full">
                                            ...
                                            <select name="category" id="" v-model="category" required class="text-sm px-2 py-1 rounded-md h-7 w-full">
                                                <option value=""></option>
                                    ...
                                    <div class="block mt-3">
                                        <div class="block text-left lg:flex lg:space-x-3 w-full">
                                            ...
                                            <input type="number" required v-model="amount" name="amount" min="0" class="text-sm rounded-md h-7 w-full">
                                        </div>
                                    </div>
                                    <div class="block mt-3">
                                        <div class="block text-left lg:flex lg:space-x-3 w-full">
                                            <label class="w-52" for="">Entry Date</label>
                                            <input type="date" required v-model="entryDate" :max="maxExpenseEntryDate" name="entry_date" class="text-sm rounded-md h-7 w-full">
                                        </div>
                                    </div>
                                </div>
                            </div>
...
export default {
    ...
    methods: {
        ...
        {
            this.id = expense.id;
            this.category = expense.category_id;
            this.amount = expense.amount;
            this.entryDate = expense.entry_date;
